...
var initOnce = function() {
  ...
  // reading monConfig. It should be read not only by monitor itself, but also by an app,
  // because app should know the port, on which monitor is listening
  ...
  for (var pid in _arr) {
    ...
    if (_arr[pid].killAttempt < maxAttemptCount) {
      ...
      if (isAlive) {
        ...
        var xxx = process.kill(pid);
        log("alive, then kill " + pid + ", " + _arr[pid].killAttempt + ", " + arr[pid].path + ", ret = " + xxx);
      } else {
